...
impl ResidualAttentionBlock {
    pub fn load<R: BufRead + Seek>(
        ...
        let attn = MultiHeadAttention::new(
            Linear::new(lt("attn.query.weight")?, Some(lt("attn.query.bias")?), true),
            Linear::new(lt("attn.key.weight")?, None, true),
            Linear::new(lt("attn.value.weight")?, Some(lt("attn.value.bias")?), true),
            Linear::new(lt("attn.out.weight")?, Some(lt("attn.out.bias")?), true),
            n_heads,
        );
        let (x_attn_ln, x_attn) = if enable_x_attn {
            ...
            let x_attn = MultiHeadAttention::new(
                ...
                Linear::new(lt("cross_attn.key.weight")?, None, true),
                Linear::new(
        ...
        let mlp = MLP::new(
            Linear::new(lt("mlp.0.weight")?, Some(lt("mlp.0.bias")?), true),
            Linear::new(lt("mlp.2.weight")?, Some(lt("mlp.2.bias")?), true),
        );
        Ok(Self {
            attn_ln,
            attn,
